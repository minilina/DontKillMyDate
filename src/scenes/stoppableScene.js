 ...
 *   Llamar setupUI DESPUÉS de setupPlayer(),
...
export default class StoppableScene extends Phaser.Scene {
    ...
    _createPauseButton() {
        let btnX, btnY, scale;
        const margin = 35; // margen deseado en píxeles (sin zoom)
        ...
        if (this._isTopDown) {
            ...
            btnX = (w / 2) + ((w / 2) / zoom) - (margin / zoom);
            btnY = (h / 2) - ((h / 2) / zoom) + (margin / zoom);
            scale = 3 / zoom;
        } else {
            btnX = this.scale.width - margin;
            btnY = margin;
            scale = 3;
        }
